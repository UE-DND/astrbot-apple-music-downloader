...
class WrapperService:
    ...
    async def _ensure_docker_wrapper(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            if not result.stdout.strip():
                # 构建镜像
                self.logger.info(f"构建Docker wrapper镜像...")
                bin_dir = self.plugin_dir / "bin"
                if not (bin_dir / "Dockerfile").exists():
                    return False, f"Dockerfile不存在: {bin_dir / 'Dockerfile'}"

                result = subprocess.run(
                    ["docker", "build", "-t", self._docker_image_name, "."],
                    cwd=str(bin_dir),
                    capture_output=True,
                    text=True,
                    timeout=120
                ...
                if result.returncode != 0:
                    return False, f"构建镜像失败: {result.stderr}"
                self.logger.info(f"Docker wrapper镜像构建完成")
    ...
    async def login_docker_wrapper(self, username: str, password: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            # 创建2FA验证码文件路径
            code_file = rootfs_dir / "data" / "data" / "com.apple.android.music" / "code"

            result = subprocess.run(
                [
                    "docker", "run", "--rm",
                    "-v", f"{rootfs_dir / 'data'}:/app/rootfs/data",
                    "-e", f"args=-L {username}:{password} -F -H 0.0.0.0",
                    self._docker_image_name
                ],
                capture_output=True,
                text=True,
            ...
            if result.returncode != 0:
                # 检查是否需要2FA
                if "2fa" in result.stderr.lower() or "verification" in result.stderr.lower():
                    return False, f"需要2FA验证码，请将验证码写入: {code_file}"
                return False, f"登录失败: {result.stderr}"

            self.logger.info(f"Docker wrapper登录成功")
            return True, "登录成功"
    ...
    async def check_docker_wrapper_logged_in(self) -> bool:
        ...
        # 检查是否存在登录凭据
        return (creds_dir / "adi.pb").exists() or (creds_dir / "files").exists()
