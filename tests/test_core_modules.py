...
from services.wrapper_manager_native import (
    WrapperProxy,
    WrapperProxyConfig,
    InstanceManager,
    WrapperInstance,
...
from services.wrapper_manager_native.instance_manager import InstanceStatus
...
async def test_wrapper_proxy_decrypt_success(wrapper_proxy):
    ...
    # Mock successful decrypt response
    with patch('aiohttp.ClientSession.post') as mock_post:
        mock_response = AsyncMock()
        mock_response.status = 200
        mock_response.read.return_value = b"decrypted_data"
        mock_response.__aenter__.return_value = mock_response
        mock_response.__aexit__.return_value = None
...
async def test_dispatcher_initialization(dispatcher):
    ...
    assert len(dispatcher._sticky_routing) == 0
...
async def test_dispatcher_dispatch_no_instances(dispatcher):
    ...
    assert "no available instance" in result.error.lower()
