...
func loadConfig() error {
	cfgPath := os.Getenv("AMDL_CONFIG_PATH")
	if cfgPath == "" {
		cfgPath = "config.yaml"
	}
...
func main() {
	...
	nonInteractiveEnv := os.Getenv("AMDL_NON_INTERACTIVE")
	nonInteractive := nonInteractiveEnv == "1" || strings.EqualFold(nonInteractiveEnv, "true")

	maxRetriesEnv := os.Getenv("AMDL_MAX_RETRIES")
	maxRetries := 1
	if maxRetriesEnv != "" {
	...
	for {
		...
		if nonInteractive {
			...
			if retryCount >= maxRetries {
				fmt.Printf("Error detected, max retries (%d) reached. Exiting.\n", maxRetries)
				os.Exit(1)
			}
			fmt.Printf("Error detected, auto-retrying (%d/%d)...\n", retryCount, maxRetries)
			counter = structs.Counter{}
			continue
		}
